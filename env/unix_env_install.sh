...
check_and_install() {
    ...
    if command -v $check_cmd >/dev/null 2>&1; then
        ...
        read -p "$name not installed, install now? (y/n) " RESP
        if [ "$RESP" = "y" ]; then
...
echo "   a) If You're using Linux - GIT"
echo "1. Homebrew (tools for easy to install different tools and library)"
echo "2. CMake (if Linux also required make and g++)"
echo "3. Vulkan SDK"
...
then
    #install git
    echo "0a. Checking GIT..."
    GIT_INSTALLED=FALSE
    check_and_install "GIT" git "sudo apt install git" GIT_INSTALLED
fi
echo
...
check_and_install "CMake" cmake "brew install cmake" CMAKE_INSTALLED

if [[ "$SYSTEM_TYPE" == "Linux" ]];
then
    check_and_install "Make" make "sudo apt install make" MAKE_INSTALLED
    check_and_install "g++" g++ "sudo apt install g++" G_INSTALLED
fi

#check vulkan sdk
VULKAN_SDK_INSTALLED=FALSE
echo "3. Checking Vulkan SDK..."
if command -v vkvia >/dev/null 2>&1;
then
